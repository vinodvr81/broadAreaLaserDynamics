...
piagr(1:dimx,1:dimz) = 0.;
 piagr1(1:dimp,1:dimz) = 0.;
 piasa(1:dimx,1:dimz) = 0.;
%   piagr(1:dimx,1:dimz) = 1.44;
%  piasa(1:dimx,1:dimz) = 0.5;
% % %  
%piagr1(1:dimx,1:3*dimz/4) =1.44;



 piasa(1:dimx,(3*dimz/4)+1:dimz) = 0.5;
% for j = 1:dimx
% for i = 1:3*dimz/4    
% piagr(j,i) = 1.47;
% end
...
for i=1:floor(dimx/2)
         pkx(i) = i * dkx; 
end
for i=floor(dimx/2)+1:dimx
         pkx(i) = (dimx - i) * dkx;
end
...
power(1:dimp) = 0.;
 ...
 piagr(1:dimx,1:3*dimz/4) = k*0.04; 
 piagr1(k,1:3*dimz/4) = piagr(1:dimx,1:3*dimz/4);
 loopnum = 0;
...
dcarA= dcarA ;
  ...
  for k = (dimp/2)+1:dimp  
         ...
         piagr(1:dimx,1:3*dimz/4) =  piagr(1:dimx,1:3*dimz/4)- 0.04;
         piagr1(k,1:3*dimz/4) = piagr(1:dimx,1:3*dimz/4);
         loopnum = 0;
...
%  power(k)=ef(dimx,dimz);
  
ef= ef;
DcarG= DcarG;
dcarA= dcarA ;
  
% ef= ef + (0.0001+0.0001*1i);
% DcarG= DcarG +0.0001;
% dcarA= dcarA +0.0001;
 end 
